...
void loop() {
  ...
  gyro_y -= gyro_y_cal;                                                //Subtract the offset calibration value from the raw gyro_y value                                               //Subtract the offset calibration value from the raw gyro_z value

  //Gyro angle calculations
  //0.0000611 = 1 / (250Hz / 65.5)                                     //Refresh rate of 250Hz
  angle_x += gyro_x * 0.0000611;                                   //Calculate the traveled pitch angle and add this to the angle_pitch variable
  angle_y += gyro_y * 0.0000611;                                   //Calculate the traveled roll angle and add this to the angle_roll variable
  
  //0.000001066 = 0.0000611 * (3.142(PI) / 180degr) The Arduino sin function is in radians
  angle_x += angle_y * sin(gyro_z * 0.000001066);               //If the IMU has yawed transfer the roll angle to the pitch angel
  angle_y -= angle_x * sin(gyro_z * 0.000001066);               //If the IMU has yawed transfer the pitch angle to the roll angel
